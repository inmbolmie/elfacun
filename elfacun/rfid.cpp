...
boolean programRfidTag(int piece) {
  ...
  if (!xSemaphoreTake( xScreenSemaphore, ( TickType_t ) screenSemaphoreTimeout )) {
    ...
    if (serialLog) Serial.println("XXXXXXXXXXXXXXX   ERROR TAKING SEMAPHORE AT programRfidTag, skipping ");
...
boolean readRfidTag(byte* liftedPiece) {
  ...
  if (!xSemaphoreTake( xScreenSemaphore, ( TickType_t ) screenSemaphoreTimeout )) {
    isSemaphoreTaken = false;
    if (serialLog) Serial.println("XXXXXXXXXXXXXXX   ERROR TAKING SEMAPHORE AT readRfidTag, skipping ");
